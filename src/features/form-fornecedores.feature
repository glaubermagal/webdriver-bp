Feature: Submitting "Cadastro do fornecedor" form
    ...
    Scenario: Test fields with invalid data (CNPJ, CNAE, Faturamento Mensal, Capital social atualizado, CPF, CEP, Site)
        When  I add "00.000.000/0000-00" to the inputfield "#content input[name=cnpj]"
            And   I add "Skynet Bots & Co." to the inputfield "#content input[name=razao-social]"
            And   I add "Terminator" to the inputfield "#content input[name=nome-fantasia]"
            And   I add "___" to the inputfield "#content input[name=cnae]"
            And   I add "WEB DESIGN" to the inputfield "#content input[name=ramo-negocio]"
            And   I add "-10000000000000000000" to the inputfield "#content input[name=faturamento-mensal]"
            And   I add "-10000000000000000000" to the inputfield "#content input[name=capital-social]"
            And   I add "Sarah Connor" to the inputfield "#content input[name=nome-responsavel]"
            And   I add "000.000.000-00" to the inputfield "#content input[name=cpf]"
            And   I add "Travessa The Cure, S/N" to the inputfield "#content input[name=endereco]"
            And   I add "Bairro? Aqui é zona rural" to the inputfield "#content input[name=bairro]"
            And   I add "000" to the inputfield "#content input[name=cep]"
            And   I add "Queimadas" to the inputfield "#content input[name=cidade]"
            And   I select the option with the value "PB" for element "#content select[name=estado]"
            And   I add "21" to the inputfield "#content [name=ddd]"
            And   I add "33220000" to the inputfield "#content [name=phone-number]"
            And   I add "theend@skynet.me" to the inputfield "#content [name=email]"
            And   I add "|/__\|" to the inputfield "#content [name=website]"
            And   I click on the element "#optante-cprb-nao"
            And   I click on the element "#label-coc"

            And   I click on the button "#content .button.medium"
        Then  I wait on element ".popup-message" to not be visible
            And   I expect that element "#content [name=cnpj]" has the class "invalid"
            And   I expect that element "#content [name=cnae]" has the class "invalid"
            And   I expect that element "#content [name=faturamento-mensal]" has the class "invalid"
            And   I expect that element "#content [name=nome-responsavel]" has the class "invalid"
            And   I expect that element "#content [name=cpf]" has the class "invalid"
            And   I expect that element "#content [name=cep]" has the class "invalid"
            And   I expect that element "#content [name=website]" has the class "invalid"
